...
- [ ] Configure cookie auth events so unauthenticated requests under `/api/*` return **401**, not the default 302 → `/signin-google`. Otherwise WASM `fetch` will follow the redirect and the JSON deserializer will choke on the HTML login page. (Verified during Phase 3 smoke test — `curl /api/user/me` without a cookie returns 302.)
