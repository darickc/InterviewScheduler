...
- [ ] DTOs in `Shared/Dtos/`: `ContactDto`, `LeaderDto`, `AppointmentTypeDto`, `AppointmentDto`, `CreateScheduleRequest`, `CreateScheduleResult`, `CalendarInfoDto`, `TimeSlotDto`
- [ ] Move `ISmsService` + `SmsService` from `src/InterviewScheduler.Infrastructure/Services/SmsService.cs` → `src/InterviewScheduler.Shared/Services/`
- [ ] Update Infrastructure to no longer depend on SmsService (only Web/Client use it now)
- [ ] Move `SchedulingRulesService` (pure logic) from Infrastructure → Shared
- [ ] Add validators / phone-number sanitization helpers used by both client and server
...
- [ ] Set `Shared.csproj` to `net9.0` to match Client; verify `CsvHelper`, `TimePeriodLibrary` still resolve in WASM context
